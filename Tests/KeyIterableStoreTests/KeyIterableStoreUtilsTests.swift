...
class KeyIterableStoreUtilsTests: XCTestCase {
	func testDefault() async throws {
		let key = "key"
		let store = Reference<[String: Int]>().keyIterableStore().default(42)
	...
	func testMap() async throws {
		...
		let key = "key"
		let store = Reference<[String: Int]>([key: 7]).keyIterableStore().map(conversion)
	...
	func testMapError() async throws {
		...
		let key = "key"
		let store = KeyIterableStore<Void, String, Int>.error(ValueStoreError.noData).mapError { _ in
			TestError.expected
		}
	...
	func testLoadDefault() async throws {
		let key = "key"
		let store = Reference<[String: Int]>([key: 42]).keyIterableStore()
