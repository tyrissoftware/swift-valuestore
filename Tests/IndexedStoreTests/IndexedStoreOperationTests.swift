...
class IndexedStoreOperationTests: XCTestCase {
	func testCopy() async throws {
		let key = "key"
		let source = Reference<[String: Int]>([key: 7]).indexedStore()
		let target = Reference<[String: Int]>().indexedStore()
	...
	func testCache() async throws {
		let key = "key"
		let slow = Reference<[String: Int]>([key: 7]).indexedStore()
		let fast = Reference<[String: Int]>([key: 1]).indexedStore()
